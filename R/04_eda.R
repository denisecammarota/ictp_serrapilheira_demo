...
## NAs and zeroes --------------------------------------------------------------
apply(iris, 2, function(x) sum(is.na(x))) #NA
...
freq_sl[1] #most frequent value for sepal length
...
cv <- function(x){ #coefficient of variation cv
  sd(x) / mean(x) * 100
}
apply(vars, 2, cv)
...
hist(iris$Petal.Length)
#set to the default way of plotting only one graph
par(mfrow = c(1, 1))
