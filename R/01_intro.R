...
mean(heights)
max(heights)
mean(heights, na.rm = TRUE) #excluding na
max(heights, na.rm = TRUE) #excluding na
...
animals_list[[1]] #double brackets for a list

# Importing data ---------------------------------------------------------------

# Reading data using read.csv
surveys <- read.csv(file = "./data/raw/portal_data_joined.csv") #using the relative path is always very important
...
all(surveys == surveys_check,na.rm=TRUE)
