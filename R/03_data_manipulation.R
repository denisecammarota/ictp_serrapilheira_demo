...
for (i in 1:length(files_path)) {
  data <- read.csv(files_path[[i]]) #auxiliary varible
  assign(file_names[i], data) #assigning actually
}
#up until here, we read and created the variables where we will keep everything
...
head(comm)
dim(comm) #97 sites and 57 species
summary(comm)

# Understanding the object `coord`
head(coord)
dim(coord) #97 sites and 3 spatial coordinates
summary(coord)
...
head(splist)
dim(splist) #56 species
summary(splist)

# Understanding the object `traits`
head(traits)
dim(traits) #56 species and 15 informative traits
summary(traits)
